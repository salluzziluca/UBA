...
$$E_{x}= \frac{-v_{i+2}+4v_{i+1}-3v_{i}}{2(x_{i+1}-x_{i})}=\frac{10,05V + 4.10,62V-3.11,13V}{2(0,02m-0,19m)} = 269 \frac{V}{m} $$
$$E_{y}= \frac{-v_{i+1}-v_{i-1}}{(y_{i+1}-y_{i-1})}=-\frac{4,96 V- 5,62 V}{0,09m-0,07m} = 33,5 \frac{V}{m}$$
$$\bar{E}(0,2m; 0,08 m) =269\frac{V}{m} \hat{\imath}+35,5 \frac{V}{m} \hat{\jmath} $$
